...
在没有模板引擎的年代，前端想要把数据渲染到页面，绝大部分都是直接操作 DOM 来实现的；可直接操作 DOM 不仅麻繁琐而且性能低下，同时还将视图和业务逻辑（数据）完全耦合在一起。
...
3. `new Function()` 传递参数问题
   ...
   那要如何解决呢？解决办法也很简单，利用 js 中一个扩展作用域（或者叫拦截作用域）方法 `with(){}` 即可实现，`with(obj) { xxx }` 方法可将包裹其中的代码的上一级作用域指向 `obj`，如：
